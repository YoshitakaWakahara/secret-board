...
存在しない場合や異常なものである場合には、再度生成してCookieに付与してその値を返す
@param {Cookies} cookies
@param {String} userName
@return {String} trackingID
*/
function addTrackingCookie(cookies, userName) {
  const requestTrackingId = cookies.get(trackingIDKey);
  if (isValidTrackingId(requestTrackingId, userName)) {
    return requestTrackingId;
  } else {
    const originalId = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
    const tomorrow = new Date(Data.now() + (1000 * 60 * 60 * 24));
